...
### 1.核心开发板：ESP32S3
尺寸结构：
- ESP32-S3 最小系统板采用矩形 PCB 外形，整体尺寸为 57.15 mm × 27.94 mm（长 × 宽）。在 PCB 两条长边上分别布置 1 列通孔排针焊盘，左右对称分布；每列排针包含 22 个引脚，引脚节距为 2.54 mm（0.1 inch），符合标准 DIP/排针封装规范。两侧排针中心线之间的横向间距（板宽方向）为 25.40 mm（1.0 inch），便于与标准面包板、扩展底板或转接板进行机械与电气兼容。
...
尺寸结构：
- TFT 显示屏的标称尺寸为 3.5 英寸。模块仅在宽边一侧集成+ 1 列排针接口，用于电源与信号连接。屏幕的整体外形尺寸为 55.5 mm（W）× 98.0 mm（H）× 14.28 mm（D），其中厚度尺寸 14.28 mm 已包含 14Pin 排针接口的高度。此外，PCB 四个角均预留标准安装通孔，可配合铜柱进行机械固定，便于系统集成与结构安装。
### 3.外置ADC介绍

#### 1.芯片型号：ADS8681 

尺寸结构：
- 该外置 ADC 模块采用矩形 PCB 结构，其整体外形尺寸为 40.00 mm × 33.00 mm。四个角均设置有定位安装孔，用于结构固定与装配
> 分辨率：16 bit 
> 采样速率：1 MSPS (最大值) 
> 基准电压：内部 4.096V 
> 通道数：单通道 
...
对物质学习与校准一栏的UI功能使用SquareLine软件进行设计，并且使用了操作pram内存的方法进行了物质存储的开辟，实现了断电数据也不会丢失的特性，
### 9.在实验环境下测试功能
测试方案：
- 1.发出波形是3.3V 频率33Hz，脉宽500us
- 2.漂移管电压：2000V
    电离电压：6000V
    离子门脉冲电压：400V

- 3.采集到的电压信号屏幕和示波器对比（不同的微电流放大器倍数）
  
测试结果（微电流放大器倍数为10^9）：
![alt text](2.png)![alt text](3.png)
最终测试结果显示屏幕显示波形和示波器波形基本一致，如图示波器显示起始0时刻到离子峰的漂移时间是5ms，屏幕显示波形与其一致。只有电压高度与示波器显示不同，怀疑是示波器内部对采集信号的放大倍数设置有问题。可以见得我们的离子迁移谱整个功能开发成功。
### 10.整体结构与外壳设计
#### 1.结构设计
在将离子迁移谱分析仪的功能全部实现后，接下来要将esp32单片机，外置ADC采集模块，3.5英寸TFT屏幕这几个主要模块集成在一块电路板上，以实现结构稳固，空间利用合理的目的。首先我将这几个模块的数据手册找到，翻阅其尺寸图，并且进行实际尺寸测量，最终精准无误的每个模块在PCB上引脚插孔都能准确对齐。再利用铜柱进行结构加固，最终PCB设计图和实物结构如下：

#### 2.外壳设计
同样的，在PCB结构设计完成之后，再次对整体空间进行测量，以及对输入输出口也进行开孔挖槽，最后使用3D打印制作外壳，其3D外壳图纸和实物如下图所示：

### 11.增加WiFi功能实现上位机远程控制采集
#### 1.摘要
本模块实现了 ESP32-S3 与 Web 前端的实时双向通信，支持离子迁移谱（IMS）数据的无线推送与远程控制。选择 AP 模式而非 STA 模式：考虑到 IMS 设备可能在无现有 WiFi 基础设施的实验室或野外使用，设备作为热点可确保"零配置"连接，用户只需连接设备 WiFi 即可访问。