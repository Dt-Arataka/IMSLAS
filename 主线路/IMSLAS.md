...
## 二、项目概况
### 1.**基于离子迁移谱的自主化学源定位系统（IMSLAS-Localization and Autonomous Seeking)**
...
### 2.离子迁移谱分析仪嵌入式系统开发
本项目旨在设计并开源一种用于离子迁移谱（IMS）分析仪的低成本、便携式嵌入式控制与数据采集系统。系统以高性能双核 MCU ESP32-S3-WROOM-1-N16R8 为核心，利用 PlatformIO 环境与 Arduino 框架开发。该系统集成了高压脉冲控制、高精度信号采集、实时谱图处理及人机交互功能，旨在替代传统IMS设备中昂贵且体积庞大的FPGA或工控机方案。核心功能分为三个模块如下：
- 模块一：人机交互与数据处理。采用 3.5 英寸 TFT 触摸屏构建交互界面。该模块负责实时绘制离子迁移谱图，并基于算法自动计算离子峰的漂移时间和电压峰值，从而实现物质识别。系统内置数据库功能，支持样本数据的本地存储与建立物质指纹对比库。
- 模块二：高保真信号采集。系统输入端接收来自跨阻放大器的模拟电压信号。为了捕捉微弱的离子流信号，采用了外部高精度、高采样率的 ADC 模块（ADS8681）。通过优化的采样算法，确保了在高速流转下的信号完整性与高信噪比。 
- 模块三：时序控制与同步。这是系统的核心控制单元，负责输出低压方波信号以驱动高压开关，从而精确控制离子门的开启与闭合。当MCU发出离子门开启信号的瞬间，ADC采集任务被硬件中断立即触发。这种严苛的同步机制消除了时序抖动，确保了每一帧谱图的漂移时间零点准确无误，从而保证了定性分析的精度。
